...
Abans de crear la xarxa neuronal, vam definir què prediuria exactament: les notes finals de matemàtiques dels alumnes. Les dades es recolliren mitjançant un formulari amb les següents preguntes:
...
També controlo l’aleatorietat de les diferents llibreries: \texttt{NumPy}, \texttt{TensorFlow}, \texttt{random}, i estableixo el valor del hash de Python amb \texttt{os}.
...
En la figura 5.18, podem observar com ha quedat la taula després d'afegir els pesos als valors d'entrada, he afegit 6 columnes de pesos respecte a les 6 entrades i una columna més pel biaix.
...
Després d'aplicar la fórmula, obtindrem la predicció de la nota final, tal com es veu en la figura 5.19.
...
Es pot veure en la imatge 5.20 dues columnes addicionals, la columna ``error'' emmagatzema els errors del model i l'altre conté els mateixos errors, però en valor absolut, és a dir, que tots els valors estan en positiu, d'aquesta manera serà més fàcil apreciar la diferència dels errors en les prediccions i els càlculs dels paràmetres.\\
...
En la figura 5.21 es pot veure com han quedat aquestes taules, els canvis del biaix es calculen amb la mateixa fórmula que els pesos, però sense restar-li cap entrada, ja que actua com una entrada constant.\\
...
Ja podem començar a entrenar el model, ho farem copiant la mateixa taula que tenim a sota, com es veu en la figura 5.22.
...
Una vegada fetes les pràctiques, compararem els següents aspectes per determinar quina de les dues formes és millor:

\begin{itemize}

 \item \textbf{Precisió: } Un dels factors més importants a l'hora de crear una xarxa neuronal predictiva és el seu percentatge de precisió, en aquest cas, la precisió de la xarxa neuronal feta per Python (93,75\%) ha sigut molt més alta que el del full de càlcul (66,7\%)

 \item \textbf{Coneixements previs: } Fer una XNA amb Python requereix coneixements de programació, aprendre el llenguatge és indispensable per crear una xarxa neuronal, en canvi, en la XNA feta amb fulls de càlculs no ha calgut cap mena de coneixements de programació.

  \item \textbf{Facilitat: } Segons la experiència de cadascun de nosaltres, fer una XNA amb fulls de càlculs és molt més fàcil que fer-ho amb Python, a banda que no cal saber programar, el procés és molt més intuïtiu i mecànic, mentre que en Python has d'anar codificant-lo tot manualment.

  \item \textbf{Tardança: } El temps que es requereix en fer una XNA amb Python depèn de l'habilitat que en tingui un en la programació, si la persona és àgil i coneix bé el llenguatge pot fer-la en molt poc temps, mentre que en un full de càlcul, malgrat que les dades i els càlculs s'obtenen automàticament, les etapes es fan manualment, cosa que es torna repetitiva i requereix molt de temps.


  \item \textbf{Visualització: } Una XNA construït per un llenguatge de programació està tot automatitzat, la qual cosa, no mostraria res de tot el procés que ha elaborat fins al resultat, en canvi, al full de càlculs, li passa tot el contrari, es construeix tot manualment, això vol dir que et mostra tots els càlculs que calcula.
