...
A l'hora de crear un projecte en equip, pot arribar a ser complicat treballar amb 2 o més membres del grup a la vegada en els seus respectius dispositius si no s'escull una eina col·laborativa adequada, sobretot si s'està treballant amb molts fitxers. En el nostre cas, vam escollir el software de git.
\subsection{Git}
Git és un software que es va desenvolupar a l'any 2005 per Linus Torvalds. Es va crear amb el proposit de mantenir un seguiment de les modificacions realitzades en arxius digitals i facilitar la colaboració entre les persones que treballen conjuntament en els mateixos arxius dins d'un repositori compartit.
...
El llenguatge de programació que vam decidir per fer la part pràctia del treball va ser Python. Python és un llenguatge de programació que va ser creat en 1989 per Guido van Rossum , un informàtic neerlandès. Aquest llenguatge és conegut per la seva simple sintaxi, comparada amb altres llenguatges com JavaScript o C++, python facilita l'aprenentatge pels usuaris que començen a apendre a programar desde 0. Aquesta va ser la principal raó per la que vam escllir python com a llenguatge de programació en la part pràctica del nostre treball, pero encara hi ha més.

\subsection{Perquè vam escollir Python?}
Voliem reduïr al màxim el nostre temps de trevall per estar més còmodes i no estar molt apresurats amb el temps. Les avantatges que té python que afavoreixen les nostres necesitats són:
\begin{itemize}
...
\end{itemize}
\subsection{Sintaxi}
Acabem d'explicar que la sintaxi de Python és simple, pero què diferència aquesta sintaxi amb altres llenguatges? La principal raò és que el seu format de codi és visualment ordenat, utiliza paraules claus de l'anglès. A diferència d'altres llenguatges, no utilitza claudàtors per determinar blocs, com per exemple les sentències ``if, else'', per exemple:
