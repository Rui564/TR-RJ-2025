\documentclass[a4paper,12pt]{book}
\usepackage{enumitem}
\usepackage{url}
\usepackage{hyperref}
\usepackage{amsmath}
\usepackage[catalan]{babel}
\usepackage{xcolor}
\usepackage{graphicx}
\usepackage{comment}
\usepackage{listings}
\usepackage{array}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{float}
\usepackage{amsfonts}
\usepackage[most]{tcolorbox}
\usepackage{setspace}

\lstset{
    literate=  % Defineix com traduir caràcters especials
        {á}{{\'a}}1 {é}{{\'e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {ú}{{\'u}}1
        {Á}{{\'A}}1 {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {Ú}{{\'U}}1
        {à}{{\`a}}1 {è}{{\`e}}1 {ì}{{\`i}}1 {ò}{{\`o}}1 {ù}{{\`u}}1
        {À}{{\`A}}1 {È}{{\'E}}1 {Ì}{{\`I}}1 {Ò}{{\`O}}1 {Ù}{{\`U}}1
        {ä}{{\"a}}1 {ë}{{\"e}}1 {ï}{{\"i}}1 {ö}{{\"o}}1 {ü}{{\"u}}1
        {Ä}{{\"A}}1 {Ë}{{\"E}}1 {Ï}{{\"I}}1 {Ö}{{\"O}}1 {Ü}{{\"U}}1
        {â}{{\^a}}1 {ê}{{\^e}}1 {î}{{\^i}}1 {ô}{{\^o}}1 {û}{{\^u}}1
        {Â}{{\^A}}1 {Ê}{{\^E}}1 {Î}{{\^I}}1 {Ô}{{\^O}}1 {Û}{{\^U}}1
        {ã}{{\~a}}1 {ẽ}{{\~e}}1 {ĩ}{{\~i}}1 {õ}{{\~o}}1 {ũ}{{\~u}}1
        {Ã}{{\~A}}1 {Ẽ}{{\~E}}1 {Ĩ}{{\~I}}1 {Õ}{{\~O}}1 {Ũ}{{\~U}}1
        {ç}{{\c c}}1 {Ç}{{\c C}}1
        {œ}{{\oe}}1 {Œ}{{\OE}}1 {æ}{{\ae}}1 {Æ}{{\AE}}1
        {ß}{{\ss}}1 {«}{{\guillemotleft}}1 {»}{{\guillemotright}}1
        {ñ}{{\~n}}1 {Ñ}{{\~N}}1 {¿}{{?`}}1 {¡}{{!`}}1,
    inputencoding=utf8,
    extendedchars=true,
    basicstyle=\ttfamily,
    breaklines=true,
    showstringspaces=false
}

% We're working with A4, which is 21cm by 29.7
\setlength{\paperwidth}{210mm}
\setlength{\paperheight}{297mm}
%\setlength{\hoffset}{-15mm}
\setlength{\voffset}{-25mm}
\setlength{\textheight}{242mm}
\setlength{\topmargin}{10mm}
\setlength{\headheight}{10mm}
\setlength{\headsep}{10mm}
\setlength{\footskip}{1.2cm} %def=30pt=1.054cm

\setlength{\textwidth}{165mm}
\setlength{\oddsidemargin}{-2.5mm}
\setlength{\evensidemargin}{-2.5mm}

% Espai entre línies de 1.5
\renewcommand{\baselinestretch}{1.5}

\usepackage{fancyhdr}
\pagestyle{fancy}
\pagenumbering{arabic}
\setlength{\parindent}{0truecm}
\lhead{}
\chead{}
\rhead{}
\lfoot{}
\cfoot{}
\rfoot{}
\renewcommand{\headrulewidth}{0pt}



\makeindex

\usepackage{titlesec}

\titleformat{\chapter}[block]
  {\normalfont\huge\bfseries\center}{\thechapter.}{1em}{\Huge}
\titlespacing*{\chapter}{-0.5cm}{-1.5cm}{0.5cm}
\renewcommand{\chaptername}{}

\begin{document}

\input{./cover/cover}

\frontmatter

\cleardoublepage
\cfoot{\thepage}

\input{./capitols/agraiments/agraiments}\cleardoublepage
\input{./capitols/Resum/Resum}\cleardoublepage

\setcounter{tocdepth}{0}
\renewcommand\contentsname{Índex de continguts}
\tableofcontents
\label{i:tableofcontents}
\addtocontents{toc}{\protect\bigskip\protect\textbf{Capítols:}\par}
\mainmatter
\pagestyle{fancy}
\fancyhf{}
\fancyhead[RE,LO]{Xarxes Neuronals}
\fancyhead[LE,RO]{\leftmark}
\fancyfoot[RE,LO]{Institut Sants}
\fancyfoot[LE,RO]{\thepage}
\renewcommand{\headrulewidth}{2pt}
\renewcommand{\footrulewidth}{1pt}


\input{./capitols/introduccio/introduccio}

\input{./capitols/objectius/objectius}

\input{./capitols/recercaPrevia/recercaPrevia}
%
\input{./capitols/metodologia/metodologia}
%
%\input{./capitols/part_pràctica/part_pràctica}
%
\input{./capitols/Resultats/Resultats}
%
\input{./capitols/conclusions/conclusions}



\appendix
\addtocontents{toc}{\protect\bigskip\protect\textbf{Apèndixs:}\par}
% \input{./capitols/rutinesLaTeX/rutinesLaTeX}
%
% \input{./capitols/codiJAVA/codiJAVA}
%
% \input{./capitols/bitacola/bitacola}
\input{./capitols/Optimització i Ajustos/Optimització i Ajustos}
% \input{./capitols/Hardware/Hardware}
\input{./capitols/Xarxa_Neuronal/Xarxa_Neuronal}
\input{./license/fdl-1.3}

\renewcommand\bibname{Bibliografia}
\bibliographystyle{./bibliografia/caplain}
\bibliography{./bibliografia/myBiblio}
\addcontentsline{toc}{chapter}{Bibliografia}
\label{b:bibliography}

\listoffigures
\listoftables




\end{document} 
